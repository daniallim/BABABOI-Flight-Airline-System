...
public class EdgeManagement extends JFrame {
    ...
    private JPanel createTablePanel() {
        ...
        tableModel = new DefaultTableModel(
                new Object[]{"NO", "From", "To", "Flight Numbers", "Distance (km)", "Time (hour)"}, 0
        ) {
            @Override
    ...
    private JPanel createAddPanel() {
        ...
        addFormPanel.add(labeled("Flight Number (e.g., AA1234)", flightNumberField));
    ...
    private int[] getSelectedPair() {
        ...
        AirportCell fromCell = (AirportCell) tableModel.getValueAt(row, 1);
        AirportCell toCell = (AirportCell) tableModel.getValueAt(row, 2);
    ...
    private void updateSelectionState() {
        ...
        if (hasSelection) {
            AirportCell fromCell = (AirportCell) tableModel.getValueAt(row, 1);
            AirportCell toCell = (AirportCell) tableModel.getValueAt(row, 2);
            String flightNo = (String) tableModel.getValueAt(row, 3);
            double dist = (double) tableModel.getValueAt(row, 4);
            double time = (double) tableModel.getValueAt(row, 5);
    ...
    // VALIDATION HELPERS
    private boolean isValidFlightNumber(String flightNo) {
        // Exactly 2 uppercase letters followed by 4 digits
        return flightNo.matches("^[A-Z]{2}[0-9]{4}$");
    }
    ...
    private void handleAddEdge() {
        ...
        String flightNo = flightNumberField.getText().trim().toUpperCase();
        if (flightNo.isEmpty()) {
            showStatus("Please enter Flight Number.", ERROR);
            return;
        }

        // Validate flight number format: 2 uppercase letters + 4 digits
        if (!isValidFlightNumber(flightNo)) {
            showStatus("Format: 2 UPPERCASE letters + 4 digits (e.g., AA1234).", ERROR);
            return;
        }
        ...
        if (distanceText.isEmpty() || timeText.isEmpty()) {
            showStatus("Please enter distance and time.", ERROR);
            return;
        }

        if (!isValidNumeric(distanceText)) {
            showStatus("Distance must be numeric (e.g., 1500.5).", ERROR);
            return;
        }

        if (!isValidNumeric(timeText)) {
            showStatus("Time must be numeric (e.g., 2.5).", ERROR);
            return;
        }
        ...
        if (distance <= 0) {
            showStatus("Distance must be positive.", ERROR);
            return;
        }
        if (time <= 0) {
            showStatus("Time must be positive.", ERROR);
            return;
        }
    ...
    private void handleUpdateEdge() {
        ...
        String newFlight = updateFlightNumberField.getText().trim().toUpperCase();
        ...
        // Validate flight number format: 2 uppercase letters + 4 digits
        if (!isValidFlightNumber(newFlight)) {
            showStatus("Flight No: 2 letters + 4 digits (e.g., AA1234).", ERROR);
            return;
        }

        String distanceText = updateDistanceField.getText().trim();
        String timeText = updateTimeField.getText().trim();
