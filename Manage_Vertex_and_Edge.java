...
public class BABABOI_Flight_System {
    ...
    private static ArrayList<String> VertexNames = new ArrayList<>();
    ...
    public static void Manage_Vertex(){
        ...
        while(VertexOption != 4){
            ...
            switch (VertexOption){
                ...
                case 4:
                    if (ConfirmMessage(input, "Are you sure want to return to the main menu?")) {
                        return;
                    }
                    VertexOption = 0;
                    break;
    ...
    public static void Show_Active_Airport(){
        ...
        for(int i=1;i< VertexNames.size();i++){
            if(!VertexNames.get(i).equals("DELETED"))
                System.out.println("ID: " + i + "\tAirport Name: " + VertexNames.get(i));
        }
    }
    
    public static void Add_Vertex(String AirportName){
        VertexNames.add(AirportName);
        int newID = VertexNames.size() - 1; 

        System.out.println("\nAirport Name: " + AirportName + " is added with ID: " + newID);
    }
    
    public static void Remove_Vertex(int ID, Scanner input){
        String VertexToRemove;
        
        if(ID < 1 || ID >= VertexNames.size()){
            System.out.println("\nError: The airport is not found!");
            return;
        }
        
        VertexToRemove = VertexNames.get(ID);
        
        if(VertexToRemove.equals("DELETED")){
            System.out.println("\nThe airport " + VertexToRemove + " is already removed!");
            return;
        }
        
        if (!ConfirmMessage(input, "Are you sure want to remove airport: " + VertexToRemove + " (ID: " + ID + ")?")) {
    ...
    public static void Search_Vertex(int ID){
        ...
        clearScreen();
        System.out.println("\nAirport " + VertexNames.get(ID) + " (ID: " + ID + ")");
  
        boolean hasRoute=false;
        System.out.println("=========================");
        System.out.println("All the routes from " + VertexNames.get(ID));
        System.out.println("=========================");
        
        for(int i=1; i< VertexNames.size();i++){
            if(DistanceMatrix[ID][i] != 0 && ID != i){
                System.out.println("--> " + VertexNames.get(i) + "(Distance: " + DistanceMatrix[ID][i] + " km, Time: " + TimeMatrix[ID][i] + " hour)" );
                hasRoute = true;
            }
        }
        if(hasRoute == false)
    ...
    public static void Manage_Edge(){
        ...
        while(EdgeOption != 5){
            ...
            System.out.println("=========================");
            System.out.println("1. View All Edge");
            ...
            switch (EdgeOption){
                ...
                default:
                    System.out.println("\nInvalid option. Please select an option between 1 to 5.");
                    System.out.print("Press Enter to continue...");
                    input.nextLine();
                    break;
            } 
    ...
    public static void Add_Edge(int From, int To,double Distance,double FlightTime){
        ...
        System.out.println( "\nThe route from " + VertexNames.get(From) + "  -->  " + VertexNames.get(To) + " (Distance: " + Distance + " km, Time: " + FlightTime + " hour)");
        
    }
    
    public static void Remove_Edge(int From,int To, Scanner input){
        if(Verify_Edge(From,To) == false)
            return;
        
        if(DistanceMatrix[From][To] == 0){
            System.out.println("\nError: The route from " + VertexNames.get(From) + " to " + VertexNames.get(To) + "  does not exist. Please try again.");
            return;
        }
        
        if (!ConfirmMessage(input, "Are you sure want to remove the route from " + VertexNames.get(From) + " to " + VertexNames.get(To) + "?")) {
            System.out.println("Removal cancelled.");
            return;
        }
        ...
        System.out.println( "\nThe route from " + VertexNames.get(From) + " to " + VertexNames.get(To) + " has been removed successfully!");
    }
    ...
    public static void Update_Edge(int From, int To, double NewDistance, double NewFlightTime){
        ...
        System.out.println("\nUpdating route: " + VertexNames.get(From) + " --> " + VertexNames.get(To) + " (Distance: " + NewDistance + " km, Time: " + NewFlightTime + " hour)");
}
    
    // Verify the edge
    public static boolean Verify_Edge(int From, int To){
        if(From < 1 || From >= VertexNames.size() || To < 1 || To >= VertexNames.size()){
            System.out.println("\nError: Invalid ID! Please use the ID from the screen." );
            return false;
        }
        
        if(VertexNames.get(From).equals("DELETED") || VertexNames.get(To).equals("DELETED")){
    ...
    public static void View_Direct_Edge(){
        boolean find;
        find = false;
        ...
        for(int i = 1; i < VertexNames.size(); i++){
            if(!VertexNames.get(i).equals("DELETED")){
                for(int j = 1; j < VertexNames.size(); j++){
                    if(DistanceMatrix[i][j] > 0){
                        System.out.println( VertexNames.get(i) + "(" + i + ") --> " + VertexNames.get(j) + "(" + j + ") (Distance: " + DistanceMatrix[i][j] + " km, Time: " + TimeMatrix[i][j] + " hour)");
                        find = true;
                    }
                }
            }
        ...
        if(find == false)
        System.out.println("No direct route is available.");
    }
    
    public static boolean ConfirmMessage(Scanner scanner, String prompt) {
        while (true) {
